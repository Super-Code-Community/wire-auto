...
`)
	...
	if len(r.Transports) != 1 || r.Transports[0] != "stdio" {
		t.Fatalf("bad transports: %+v", r.Transports)
	}
}

func TestLoadRuntimeMissingProtocols(t *testing.T) {
	p := write(t, "runtime.manifest", `
name = "wire-auto-runtime"
version = "0.1.0"
transports = ["stdio"]
cores = ["regular"]
`)
	if _, err := LoadRuntime(p); err == nil {
		t.Fatal("expected error for missing protocols")
	}
}

func TestLoadRuntimeMissingTransports(t *testing.T) {
	p := write(t, "runtime.manifest", `
name = "wire-auto-runtime"
version = "0.1.0"
protocols = [1]
