...
**Безопасность для локальной сети.** «Толстый» скрипт делит сеть с остальной
машиной, поэтому конкурентность держим в узде: `workers = 100`, а на успешном
TCP-соединении вызывается `SetLinger(0)`, чтобы `Close` слал RST и эфемерный порт
освобождался сразу (без `TIME_WAIT`). Иначе полный свип 65535 портов по
неотвечающему удалённому хосту (фильтрованные порты висят до таймаута) выжигает
диапазон динамических портов и переполняет NAT роутера — кладёт сеть всего ПК.
Ход скана виден в логах `scanned N/65535`.
